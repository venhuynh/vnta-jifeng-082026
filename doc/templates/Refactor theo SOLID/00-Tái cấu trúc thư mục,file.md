...
- Tên hiển thị/nghiệp vụ: `Khấu trừ phí công đoàn`
- UI root hiện tại: `src/Vnta.HRM2026/Vnta.Hrm.Web.Client/Components/KhauTru/KhauTruPhiCongDoan`
- UI route cần giữ nguyên: `/payroll/union-fee-deductions`
- Mô tả ngắn hành vi nghiệp vụ: Chọn kỳ lương để chuẩn bị snapshot phí công đoàn, tải danh sách theo bộ lọc nhân viên/phòng ban/chức vụ với phân trang; cho phép tính lại theo dữ liệu nguồn (bỏ qua dòng đã khóa), điều chỉnh thủ công số tiền trên dòng chưa khóa, khóa/mở khóa từng dòng hoặc toàn bộ kỳ, đối chiếu bảng công tháng ở chế độ chỉ đọc và xuất toàn bộ kỳ ra Excel/PDF.
- Phạm vi thay đổi nghiệp vụ được phép: Chỉ refactor cấu trúc; giữ nguyên UX, route, authorization, audit, optimistic concurrency, lock semantics và các HTTP contract hiện có.
- Các file hoặc thay đổi đang có cần tuyệt đối giữ nguyên: Giữ nguyên toàn bộ behavior của `KhauTruPhiCongDoan.razor` và các popup chỉnh sửa, khóa/mở khóa, tính lại, quy tắc, đối chiếu bảng công.
