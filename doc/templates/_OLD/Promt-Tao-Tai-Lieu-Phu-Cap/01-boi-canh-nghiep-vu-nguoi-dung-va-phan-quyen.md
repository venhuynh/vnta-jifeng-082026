# Prompt 01 — Bối cảnh nghiệp vụ, người dùng và phân quyền
...
<THU_MUC_TAI_LIEU_DICH>/01-tong-quan-nghiep-vu.md

Phân tích mục tiêu nghiệp vụ, đối tượng dữ liệu, persona, use case, đầu vào/đầu ra và tác động downstream của màn hình.
Lần theo [Authorize], policy ở route/API và điều kiện UI để lập ma trận quyền: hành động, quyền kỹ thuật, scope dữ liệu, precondition, postcondition và bằng chứng source.
Không suy ra phân quyền dữ liệu chỉ từ việc nút bị disable.

CẤU TRÚC BẮT BUỘC
1. Metadata/phạm vi.
2. Mục tiêu nghiệp vụ và luồng đầu-cuối.
3. Persona và use case.
4. Ma trận phân quyền.
5. Dữ liệu vào/ra, downstream và giả định.
6. Bằng chứng source, mâu thuẫn/chưa xác minh.
```
