...
<THU_MUC_TAI_LIEU_DICH>/02-ui-dxtoolbar.md

Đọc Razor/CSS/code-behind và mô tả riêng toàn bộ DxToolbar/DxToolbarItem: title, vị trí, thứ tự, nhóm/dropdown, control con, tooltip, icon, responsive và điều kiện render.
Liệt kê mọi action toolbar nhưng chỉ liên kết sang các tài liệu logic action riêng, không lặp lại chi tiết xử lý.

CẤU TRÚC BẮT BUỘC
1. Kết quả tìm kiếm DxToolbar.
2. Bảng inventory DxToolbarItem: text, control, alignment, tooltip, icon, handler/liên kết logic, enabled/visible.
3. Bố cục desktop/mobile và accessibility.
4. Khác biệt baseline (nếu có) và bằng chứng source.
```
