# Prompt 06 — Mô tả UI — Toast và notification
...
<THU_MUC_TAI_LIEU_DICH>/06-ui-toast-notification.md

Truy vết service/component toast, notification, alert hoặc feedback tương đương. Lập catalog: trigger, method, mức độ (success/info/warning/error), message, dữ liệu động, thời điểm hiển thị, action tiếp theo và khả năng đóng.
Đối chiếu UI handler với error/success response; không tự chuẩn hóa message nếu source chưa có.

CẤU TRÚC BẮT BUỘC
1. Cơ chế feedback và DI/source map.
2. Toast/notification catalog.
3. Quy tắc lỗi, retry/reload và accessibility.
4. Message thiếu/không nhất quán và bằng chứng source.
```
