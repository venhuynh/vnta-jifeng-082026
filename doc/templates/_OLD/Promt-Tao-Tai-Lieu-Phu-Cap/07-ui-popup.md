...
<THU_MUC_TAI_LIEU_DICH>/07-ui-popup.md

Inventory tất cả DxPopup/dialog/drawer/component modal. Với từng popup, mô tả trigger mở, state visible, dữ liệu input/output, title, controls, validation, confirm/cancel/close, loading, focus/keyboard, kích thước/responsive và side effect khi đóng.
Bao gồm popup xác nhận, điều chỉnh, quy tắc, chi tiết và bảng công nếu tồn tại.

CẤU TRÚC BẮT BUỘC
1. Danh sách popup.
2. Ma trận popup: trigger → state → input → action → output/side effect.
3. UX/loading/error/accessibility.
4. N/A, mâu thuẫn và bằng chứng source.
```
