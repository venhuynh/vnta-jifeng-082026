...
<THU_MUC_TAI_LIEU_DICH>/08-ui-icon.md

Lập icon catalog cho toolbar, dropdown, grid row action và popup. Ghi icon source/name, control sử dụng, ý nghĩa nghiệp vụ, tooltip/accessible label, trạng thái dynamic (ví dụ lock/unlock), quy ước icon set và icon thiếu tooltip.
Không mô tả icon không xuất hiện trong source.

CẤU TRÚC BẮT BUỘC
1. Quy ước icon.
2. Bảng icon catalog.
3. Dynamic icon và trạng thái.
4. Accessibility/nhất quán UI và bằng chứng source.
```
