# Prompt 10 — Logic DxToolbar — Tính lại
...
<THU_MUC_TAI_LIEU_DICH>/10-toolbar-tinh-lai.md

Phân tích action Tính lại/Recalculate: trigger, confirmation popup, scope dữ liệu, xử lý dòng/kỳ đã khóa, command/API, công thức/service, dữ liệu ghi downstream, reload/patch local, chống double-submit và toast/error.
Nếu action không tồn tại, ghi N/A có bằng chứng.

CẤU TRÚC BẮT BUỘC
1. Trigger/enable/confirmation.
2. Sequence và scope tính lại.
3. Lock, idempotency, concurrency và side effect.
4. State/feedback/error/test.
5. Bằng chứng source.
```
