# Prompt 11 — Logic DxToolbar — Khóa
...
<THU_MUC_TAI_LIEU_DICH>/11-toolbar-khoa.md

Phân tích action Khóa ở toolbar. Mô tả selected rows so với whole period, điều kiện selection, popup xác nhận, trạng thái lock target, batch API, dữ liệu cập nhật, response đã áp dụng/không có dữ liệu, audit, reload/patch và toast.
Không gộp logic Mở khóa vào tài liệu này.

CẤU TRÚC BẮT BUỘC
1. Trigger/enable/scope.
2. Popup và validation.
3. Sequence UI → backend → persistence/audit.
4. State sau khóa, error/retry/test.
5. Bằng chứng source.
```
