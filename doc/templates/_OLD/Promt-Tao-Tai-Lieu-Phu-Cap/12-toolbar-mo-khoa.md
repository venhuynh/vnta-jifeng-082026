# Prompt 12 — Logic DxToolbar — Mở khóa
...
<THU_MUC_TAI_LIEU_DICH>/12-toolbar-mo-khoa.md

Phân tích action Mở khóa ở toolbar độc lập với Khóa. Mô tả scope, popup, validation, batch API, semantics khi dòng đã mở khóa, audit, reload/patch, toast và các thao tác được khôi phục sau mở khóa.
Nếu dùng cùng handler với Khóa, vẫn mô tả branch/parameter riêng.

CẤU TRÚC BẮT BUỘC
1. Trigger/enable/scope.
2. Popup và validation.
3. Sequence UI → backend → persistence/audit.
4. State sau mở khóa, error/retry/test.
5. Bằng chứng source.
```
