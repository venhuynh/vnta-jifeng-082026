# Prompt 15 — Logic dòng dữ liệu — Điều chỉnh
...
<THU_MUC_TAI_LIEU_DICH>/15-row-dieu-chinh.md

Phân tích action Điều chỉnh/Edit/manual update ở từng dòng: điều kiện row, popup/form, giá trị prefill, field được sửa, validation client/server, save API, lock guard, persistence/audit, reload/patch local, cancel và toast/error.
Không gộp action này với Chi tiết hoặc Tính lại.

CẤU TRÚC BẮT BUỘC
1. Trigger/enable và quyền.
2. Form/popup/field/validation.
3. Sequence save, persistence và audit.
4. Lock/concurrency/error/test.
5. Bằng chứng source.
```
