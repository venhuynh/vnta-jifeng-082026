# Prompt 17 — Logic dòng dữ liệu — Khóa/Mở khóa
...
<THU_MUC_TAI_LIEU_DICH>/17-row-khoa-mo-khoa.md

Phân tích action chuyển trạng thái Khóa/Mở khóa trên một dòng. Mô tả label/icon dynamic, điều kiện row, handler/parameter, API, optimistic/concurrency behavior, audit, patch/reload, popup nếu có và những action bị chặn/được mở lại.
Tách rõ row action với batch toolbar action.

CẤU TRÚC BẮT BUỘC
1. Trigger/icon/enable.
2. Sequence một dòng và dữ liệu cập nhật.
3. State machine lock của row.
4. Audit/error/retry/test.
5. Bằng chứng source.
```
