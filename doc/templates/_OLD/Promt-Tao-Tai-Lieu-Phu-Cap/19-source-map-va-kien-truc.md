# Prompt 19 — Source map và kiến trúc UI → API → backend → database
...
<THU_MUC_TAI_LIEU_DICH>/19-source-map-va-kien-truc.md

Lập source map đầy đủ: Razor, code-behind, CSS, popup, model, provider, API client, DI, endpoint, application service, infrastructure, entity, migration, test và audit.
Vẽ data lineage/read-write sequence cho từng command quan trọng; đánh dấu legacy/dead endpoint có bằng chứng.

CẤU TRÚC BẮT BUỘC
1. Sơ đồ kiến trúc.
2. Source map theo layer.
3. Endpoint/contract/service catalog.
4. Database/data lineage/audit.
5. Dependency, legacy và bằng chứng source.
```
