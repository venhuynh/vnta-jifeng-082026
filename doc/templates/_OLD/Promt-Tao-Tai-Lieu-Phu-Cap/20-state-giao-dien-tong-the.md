# Prompt 20 — State giao diện tổng thể và chuyển trạng thái
...
<THU_MUC_TAI_LIEU_DICH>/20-state-giao-dien-tong-the.md

Tổng hợp state fields, derived state và state machine toàn màn: idle, loading, applied/pending filter, empty, error, saving, exporting, popup và lock.
Lập bảng state → control enabled/visible và sơ đồ transition; tham chiếu các tài liệu action 09–18 thay vì lặp lại chi tiết logic.

CẤU TRÚC BẮT BUỘC
1. State inventory.
2. Derived state/guard.
3. State transition diagram.
4. Ma trận enabled/visible.
5. Cancellation, debounce, double-submit và bằng chứng source.
```
