# Prompt 22 — API, persistence, transaction, concurrency và audit
...
<THU_MUC_TAI_LIEU_DICH>/22-api-persistence-transaction-audit.md

Phân tích API contract, request/response/error, bảng/entity đọc ghi, transaction, idempotency, concurrency, lock semantics, downstream consistency, schema constraints và audit.
Không gọi là atomic hoặc có optimistic concurrency nếu source không chứng minh.

CẤU TRÚC BẮT BUỘC
1. API contract catalog.
2. Command/query flows.
3. Persistence/transaction/side-effect matrix.
4. Concurrency/lock/audit.
5. Schema/migration/risk và bằng chứng source.
```
