...
- Structure đích: Feature-first dưới `Components/PhuCap/PhuCapTrachNhiemCapBac`, Application/Infrastructure shared `PhuCap/PhuCapTrachNhiem`, endpoint/API/provider tương ứng và test theo feature.
- Route/API cần giữ: `/payroll/responsibility-allowances/grades` và `/api/payroll/responsibility-allowance/grade-config*`; authorization `InternalAccountPolicies.PayrollAdministration`.
- Breaking changes được phép: Không; giữ UX, JSON/schema, status code, validation, trạng thái sử dụng/ngừng dùng và audit behavior.
