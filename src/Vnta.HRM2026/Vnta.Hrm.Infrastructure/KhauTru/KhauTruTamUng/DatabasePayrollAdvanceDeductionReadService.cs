...
namespace Vnta.Hrm.Infrastructure.KhauTru.KhauTruTamUng;

public sealed class DatabasePayrollAdvanceDeductionReadService(ApplicationDbContext dbContext)
    : IPayrollAdvanceDeductionReadService
{
    private const int MinimumSupportedYear = 1900;
    private const int MaximumSupportedYear = 2100;
    private const int MaximumPageSize = 200;

    public async Task<PayrollAdvanceDeductionPageDto> SearchAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var query =
            from detail in dbContext.PayrollDeductionAdvanceRecords.AsNoTracking()
        ...
        var rows = await query
            ...
            .Select(item => new PayrollAdvanceDeductionRow(
        ...
        return new PayrollAdvanceDeductionPageDto(rows.Select(MapToDto).ToArray(), totalCount);
    }

    private static PayrollAdvanceDeductionListItemDto MapToDto(PayrollAdvanceDeductionRow source) =>
        new(
    ...
    private sealed record PayrollAdvanceDeductionRow(
