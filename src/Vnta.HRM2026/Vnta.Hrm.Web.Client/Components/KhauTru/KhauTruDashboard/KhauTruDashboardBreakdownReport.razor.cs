...
public partial class KhauTruDashboardBreakdownReport
{
    [Parameter, EditorRequired] public IReadOnlyList<PayrollDeductionDashboardDeductionBreakdownDto> Breakdown { get; set; } = [];
    [Parameter] public bool IsRefreshing { get; set; }
    [Parameter] public EventCallback RefreshRequested { get; set; }

    private IReadOnlyList<PayrollDeductionDashboardDeductionBreakdownDto> NonZeroBreakdown =>
        Breakdown.Where(item => item.Amount != 0m).ToArray();

    private IReadOnlyList<BreakdownChartPoint> ChartPoints => NonZeroBreakdown
        .GroupBy(item => item.DeductionType)
        .Select(group => new
        {
            DeductionType = group.Key,
    ...
    private sealed record BreakdownChartPoint(string DeductionType, double Amount, string Label);
}
