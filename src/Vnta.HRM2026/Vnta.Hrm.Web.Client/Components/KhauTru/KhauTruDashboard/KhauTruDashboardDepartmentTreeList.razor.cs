...
public partial class KhauTruDashboardDepartmentTreeList
{
    ...
    private static decimal GetAmount(PayrollDeductionDashboardDepartmentTreeNodeDto row, int month) =>
        row.Months.FirstOrDefault(item => item.PayrollMonth == month)?.Amount ?? 0m;

    private static decimal GetChange(PayrollDeductionDashboardDepartmentTreeNodeDto row, int month) =>
    ...
    {
        if(args.DataItem is not PayrollDeductionDashboardDepartmentTreeNodeDto row) return;

        args.Value = GetMonthlyColumnValue(row, args.FieldName);
    }

    private static decimal? GetMonthlyColumnValue(PayrollDeductionDashboardDepartmentTreeNodeDto row, string? fieldName)
    {
    ...
    private static string GetDepartmentNameCssClass(PayrollDeductionDashboardDepartmentTreeNodeDto node) =>
