...
public partial class PhuCapDashboardMonthlyComparisonReport
{
    ...
    [Parameter, EditorRequired] public IReadOnlyList<PayrollAllowanceDashboardAllowanceComparisonDto> Comparison { get; set; } = [];
    ...
    private IReadOnlyList<ComparisonMonth> ComparisonMonths => Enumerable.Range(1, ComparisonEndMonth)
        .Select(month => new ComparisonMonth(month, $"Tháng {month:00}", $"allowance-month-{month:00}", GetMonthTotal(month)))
        .ToArray();
    ...
    private static decimal GetAmount(PayrollAllowanceDashboardAllowanceComparisonDto row, int month) =>
        row.Months.FirstOrDefault(item => item.PayrollMonth == month)?.Amount ?? 0m;

    private static decimal GetChange(PayrollAllowanceDashboardAllowanceComparisonDto row, int month) =>
    ...
    {
        if(args.DataItem is not PayrollAllowanceDashboardAllowanceComparisonDto row) return;

        args.Value = GetMonthlyColumnValue(row, args.FieldName);
    }

    private static decimal? GetMonthlyColumnValue(PayrollAllowanceDashboardAllowanceComparisonDto row, string? fieldName)
    {
